...
var DONT_HAVE_PERMISSION = 'DONT_HAVE_PERMISSION';

var MIN_HOURS_ALLOW_CANCELLATION = 'MIN_HOURS_IN_ADVANCE_TO_ALLOW_CANCELLATION_AMENDMENT_PRIOR_TO_RESERVATION_TIME';
var MIN_HOURS_IN_ADVANCE_PRIOR_TO_RESERVATION_TIME = 'MIN_HOURS_IN_ADVANCE_SLOT_TIME';
var MIN_HOURS_IN_ADVANCE_PRIOR_TO_SESSION_TIME = 'MIN_HOURS_IN_ADVANCE_SESSION_TIME';
  ...
  */
    ...
    */
      ...
      */
				...
				watch: {
					...
					weekly_sessions: function weekly_sessions(_weekly_sessions) {
						var weekly_view = self.computeWeeklyView(_weekly_sessions);
						// Update weekly view
				...
				methods: {
					...
					_updateSingleTimingArrival: function _updateSingleTimingArrival(timing, timing_property, event) {
					...
					_timingsMounted: function _timingsMounted() {
       ...
       */
        ...
        */
					...
					_updateBufferMinHoursAllowCancellationOrAmendment: function _updateBufferMinHoursAllowCancellationOrAmendment(value) {
						console.log('see update', value);
						var vue_state = window.vue_state;
						var buffer = vue_state.buffer;
						// Check min hour allow cancellation must respect
						// new booking
						var min_hours_prior_to_reservation_time = buffer[MIN_HOURS_IN_ADVANCE_PRIOR_TO_RESERVATION_TIME];
						var min_hours_prior_to_session_time = buffer[MIN_HOURS_IN_ADVANCE_PRIOR_TO_SESSION_TIME];
						var is_respect_booking_time = value >= min_hours_prior_to_reservation_time && value >= min_hours_prior_to_session_time;
						if (is_respect_booking_time) {
							buffer[MIN_HOURS_ALLOW_CANCELLATION] = value;
						} else {
							var msg = 'Please ensure that the minimum hours in advance to allow cancellation/amendment is equal or larger than the minimum hours in advance to allow booking.';
							window.alert(msg);
							// Set back to previous value
							var newBuffer = Object.assign({}, buffer);
		...
		value: function buildVueState() {
			var vue_state = this.getFrontendState();
   ...
   */
    ...
    */
		...
		value: function computeWeeklyView(weekly_sessions) {
			var today = moment();
				...
				case AJAX_UPDATE_DEPOSIT:
					{
						...
						var _data = Object.assign({}, action, { outlet_id: _outlet_id2 });
